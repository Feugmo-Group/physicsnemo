...
class c_Geometry:
    ...
    def sample_interior(
        ...
        tasks = [
            (int(nr_points * inner_ratio), inner_bounds, False),
            (nr_points - int(nr_points * inner_ratio), bounds, True),
        ]

        final_invar = {}

        for target_nr, current_bounds, exclude_inner in tasks:
            invar = {}
            total_tried = 0
            total_sampled = 0

            while total_sampled < target_nr:
                local_invar = current_bounds.sample(target_nr, parameterization, quasirandom)
                local_params = parameterization.sample(target_nr, quasirandom)

                sdf_results = self.sdf(local_invar, local_params, compute_sdf_derivatives)
                local_invar.update(sdf_results)
                if flip_interior:
                    criteria_index = np.less(local_invar["sdf"], 0)
                else:
                    criteria_index = np.greater(local_invar["sdf"], 0)

                # 排除过滤：如果是全局任务，剔除掉 inner 区域的点
                if exclude_inner:
                    is_in_inner = np.ones_like(criteria_index, dtype=bool)
                    for p, (l, h) in inner_ranges.items():
                        # 确保处理 local_invar[axis]
                        is_in_inner &= (local_invar[p.name] >= l) & (local_invar[p.name] <= h)
                    criteria_index &= ~is_in_inner

                if criteria is not None:
                    criteria &= criteria(local_invar, local_params)
                # 应用掩码并合并
                for key in local_invar.keys():
                    valid_data = local_invar[key][criteria_index[:, 0], :]
                    invar[key] = np.concatenate([invar.get(key, np.empty((0, 1))), valid_data], axis=0)

                total_sampled = next(iter(invar.values())).shape[0]
                total_tried += target_nr

                if total_sampled > target_nr * 100 and total_sampled < 1:
                    raise RuntimeError("failed")

            vol = (total_sampled / total_tried) * current_bounds.volume(parameterization)
            for key in invar.keys():
                invar[key] = invar[key][:target_nr]
            invar["area"] = np.full_like(invar["sdf"], vol / target_nr)
            invar.update(parameterization.sample(target_nr))

            for key in invar.keys():
                final_invar[key] = np.concatenate([final_invar.get(key, np.empty((0, 1))), invar[key]], axis=0)
