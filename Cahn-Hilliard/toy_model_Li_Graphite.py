...
# inputs
x_pred_val = np.linspace(0, 8e-6, 10000).reshape(-1, 1)
...
class GoverningPDE(PDE):
    def __init__(self):
        ...
        self.equations["chemical-potential"] = (
            mu - ((R * T / V_m * sympy.log(x / (1 - x))) +
             (omega_0 / V_m * (1 - 2 * x)) -
             (gamma / c_max * eta * (1 - eta)) -
             k_c * (c_li.diff(x, 2)))
        )
        self.equations["Cahn-Hilliard"] = (
            c_li.diff(y, 1) - (M(c_li) * mu.diff(x, 1)).diff(x, 1)
        )
        self.equations["Allen-Cahn"] = (
            eta.diff(y, 1) - (- L_eta * (2 * A_0 * eta * (1 - eta) * (1 - 2 * eta) - gamma * x * (1 - 2 * eta) - k_eta * eta.diff(x, 2)))
        )
