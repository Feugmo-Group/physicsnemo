...
class PDE_Function(PDE):
    def __init__(self):
        ...
        self.equations = {}
        self.equations["dc_vac_dx"] = (
            c_vac_star.diff(x, 1)
        )
        self.equations["dc_elec_dx"] = (
            c_elec_star.diff(x, 1)
        )
        self.equations["dc_yzr_dx"] = (
            c_yzr_star.diff(x, 1)
        )
        ...
        self.equations["Cahn-Hilliard"] = (
            eta.diff(y, 1) - M_star * (4 * eta * (1 - eta) * (1 - 2 * eta) - k_star * (eta.diff(x, 2))).diff(x, 2)
        )
        # Concentration Eqs
        self.equations["conc_vac"] = (
            0.01 * (c_vac_star.diff(y, 1) -
...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    boundary_loss_dc_vac = PointwiseBoundaryConstraint(
        nodes=nodes,
        geometry=rec,
        fixed_dataset=False,
        outvar={"dc_vac_dx": 0},
    ...
    PDE_domain.add_constraint(boundary_loss_dc_vac, "boundary_dc_vac")

    boundary_loss_dc_elec = PointwiseBoundaryConstraint(
        nodes=nodes,
        geometry=rec,
        fixed_dataset=False,
        outvar={"dc_elec_dx": 0},
    ...
    PDE_domain.add_constraint(boundary_loss_dc_elec, "boundary_dc_elec")

    boundary_loss_dc_yzr = PointwiseBoundaryConstraint(
        nodes=nodes,
        geometry=rec,
        fixed_dataset=False,
        outvar={"dc_yzr_dx": 0},
    ...
    PDE_domain.add_constraint(boundary_loss_dc_yzr, "boundary_dc_yzr")
