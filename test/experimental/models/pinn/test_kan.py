...
"""Tests for KolmogorovArnoldNet (MOD-008a/b/c)."""
...
_REF = _DATA_DIR / "kan_ref.pth"
_SEED = 0


def _build(config):
    if config == "default":
        return KolmogorovArnoldNet(in_features=2, out_features=1)
    return KolmogorovArnoldNet(
        in_features=3,
        out_features=2,
        layer_size=8,
        nr_layers=3,
...
def test_constructor(config):
    ...
    if config == "default":
        ...
        assert model.layer_size == 5
        assert model.nr_layers == 2
        assert model.spline_order == 3
        assert model.free_knot is False
    else:
        assert model.in_features == 3
        assert model.out_features == 2
        assert model.layer_size == 8
        assert model.nr_layers == 3
        assert model.spline_order == 2
        assert model.free_knot is True

    in_f = model.in_features
    out_f = model.out_features
    x = torch.randn(16, in_f)
    out = model(x)
    assert out.shape == (16, out_f)
...
def test_checkpoint(tmp_path):
    ...
    assert loaded.spline_order == model.spline_order
