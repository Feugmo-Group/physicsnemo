...
"""Tests for FourierKolmogorovArnoldNet (MOD-008a/b/c)."""
...
_REF = _DATA_DIR / "fourier_kan_ref.pth"
_SEED = 0


def _build(config):
    if config == "default":
        return FourierKolmogorovArnoldNet(
            in_features=2, out_features=1, layer_size=16, nr_layers=2
        )
    return FourierKolmogorovArnoldNet(
        in_features=2,
        out_features=2,
        frequencies=("gaussian", 1.0, 4),
        layer_size=8,
        nr_layers=2,
        grid_size=6,
    )
...
def test_constructor(config):
    ...
    if config == "default":
        ...
        assert model.frequencies[0] == "axis"
    else:
        assert model.out_features == 2
        assert model.frequencies[0] == "gaussian"
...
def test_checkpoint(tmp_path):
    ...
    ckpt = tmp_path / "fourier_kan.mdlus"
    model.save(str(ckpt))
